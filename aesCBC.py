...
'''
# Implement CBC by using ECB
pt[0] = cipher.decrypt(m[0:16] ^ IV)
pt[1] = cipher.decrypt(m[16:32] ^ pt[0])
pt[2] = cipher.decrypt(m[32:48] ^ pt[1])
'''
...
pt = []
a = bytes([ x ^ y for x, y in zip(m[0:16], IV)])

pt.append(cipher.decrypt(a))

for i in range(16, len(m), 16):
    b = bytes([x ^ y for x, y in zip( cipher.decrypt(m[i:i+16]), m[i-16:i] )])
    pt.append(b)
